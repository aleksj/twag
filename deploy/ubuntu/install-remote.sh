...
sudo install -d -m 0750 -o "$SERVICE_USER" -g "$SERVICE_GROUP" /var/log/twag/terminal-traces
if [[ ! -f "$ENV_FILE" ]]; then
...
if systemctl list-unit-files "twag-nimble@.service" >/dev/null 2>&1; then
  ...
  sudo rm -f "$SYSTEMD_DIR/twag-nimble@.service"
fi
...
Completed:
  ...
  - ensured /var/log/twag/terminal-traces exists
